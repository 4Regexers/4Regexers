...
> 지훈이는 증권사에 입사한 첫날이다. 첫 업무로 주식 거래 데이터베이스를 검증하고 필요한 정보를 뽑아내는 일을 담당하게 되었다. 아래 7가지 업무들을 해결하기 위해 아래정보들을 참고하여 공유 DB에 정규표현식을 활용한 select문장을 구성해보자.   
...
5. 특정 가격 범위 거래
   가진 재산이 $200.00인 고객이 $100.00이상의 주식 중 자신이 구매 가능한 종목을 문의 하였다. 거래기록을 기반으로 매수 가능한 거래 ID를 확인하여라.
